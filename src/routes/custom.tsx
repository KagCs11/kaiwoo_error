  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: "description",
        content:
          "Design your own laser engraved wooden keychain. Upload a photo, pick wood and shape, see a live preview.",
      },
    ],
  ...
  const onUpload = async (
    ...
    try {
      ...
      const res = await fetch(
        "http://localhost:5000/ai/preview",
        {
          method: "POST",
          body: formData,
        }
    ...
    <SiteLayout>
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <p className="mt-3 text-muted-foreground max-w-xl">
          Upload your favorite photo and customize every detail.
          AI preview included.
        </p>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          ...
          <div className="lg:sticky lg:top-24 h-fit">
              ...
              {loading ? (
                ...
                <div
                  className={`relative h-3/4 w-3/4 ${
                    shape === "heart"
                      ? "[clip-path:polygon(50%_15%,80%_0,100%_30%,75%_75%,50%_100%,25%_75%,0_30%,20%_0)]"
                      : shape === "circle"
                      ? "rounded-full"
                      : "rounded-2xl"
                  } overflow-hidden`}
                >
                  ...
                  <div className="absolute inset-0 mix-blend-multiply bg-walnut/20" />
                </div>
              ) : (
                <div className="text-center text-muted-foreground">
            ...
            <p className="mt-3 text-xs text-center text-muted-foreground">
              Wood: {WOODS.find((w) => w.id === wood)?.name}
              {" · "}
